...
head(targets)
colSums(targets)
length(rownames(targets)[rowSums(targets[,1:4])>0]) # 1799, not great, not terrible.
...
## Subset for Berta selection of targets, September 2020
blower.sel <- read.xlsx(file.path(.tablesdir,'hexful-list-blower-selection-sep2020.xlsx'),colNames=FALSE)
fasta <- fasta[names(fasta) %in% as.character(blower.sel[,1])]
writeXStringSet(fasta,filepath=file.path(.datadir,'blower_pas_seq/most_reads_fasta_sel_sep2020.fa'))
...
## Save
writeXStringSet(fasta.cpeb1,filepath=file.path(.datadir,'blower_pas_seq/most_reads_fasta_sel_sep2020_cpeb1.fa'))
writeXStringSet(fasta.cpeb234,filepath=file.path(.datadir,'blower_pas_seq/most_reads_fasta_sel_sep2020_cpeb234.fa'))
writeXStringSet(fasta.cpeb1234,filepath=file.path(.datadir,'blower_pas_seq/most_reads_fasta_sel_sep2020_cpeb1234.fa'))

## Redefine homer function to call options with fasta
## our global parameters
path <- file.path(.reportsdir,'homer_output_blower_rna_subset')
homer_base <- get_homer_bin()
motif_length <- c(8,10,12)
scan_size <- 'given' ## equals option -chopify
optimize_count <- 8
...
fasta <- '/Volumes/biostats/consulting/raul_mendez/bduran_201903_ripseq/data/blower_pas_seq/most_reads_fasta_sel_sep2020_cpeb1234.fa'
background <- '/Volumes/biostats/consulting/raul_mendez/bduran_201903_ripseq/data/blower_pas_seq/most_reads_fasta_sel_sep2020.fa'
path <- file.path(.reportsdir,'motifs_homer_blower_cpeb1234_vs_bg_120620_rna_sel_sep2020')
out <- run_homer_fasta(homer_base, fasta, path, background, run=TRUE)

## Launch Homer, CPEB1 vs All UTRs
fasta <- '/Volumes/biostats/consulting/raul_mendez/bduran_201903_ripseq/data/blower_pas_seq/most_reads_fasta_sel_sep2020_cpeb1.fa'
background <- '/Volumes/biostats/consulting/raul_mendez/bduran_201903_ripseq/data/blower_pas_seq/most_reads_fasta_sel_sep2020.fa'
path <- file.path(.reportsdir,'motifs_homer_blower_cpeb1_vs_bg_120620_rna_sel_sep2020')
out <- run_homer_fasta(homer_base, fasta, path, background, run=TRUE)

## Launch Homer, CPEB234 vs All UTRs
fasta <- '/Volumes/biostats/consulting/raul_mendez/bduran_201903_ripseq/data/blower_pas_seq/most_reads_fasta_sel_sep2020_cpeb234.fa'
background <- '/Volumes/biostats/consulting/raul_mendez/bduran_201903_ripseq/data/blower_pas_seq/most_reads_fasta_sel_sep2020.fa'
path <- file.path(.reportsdir,'motifs_homer_blower_cpeb234_vs_bg_120620_rna_sel_sep2020')
out <- run_homer_fasta(homer_base, fasta, path, background, run=TRUE)
