...
###########################################################################
# 2.1 GENERATE RLOG MATRIX AS USED IN DESEQ2 ANALYSIS
###########################################################################
...
# CPEB2/3/4 vs CPEB1
sampleinfo$Group2 <- as.character(sampleinfo$Group)
sampleinfo$Group2[sampleinfo$Group2 %in% sprintf('IP_CPEB%d',2:4)] <- 'IP_CPEB234'
# CPEB2/4 vs CPEB3
sampleinfo$Group3 <- as.character(sampleinfo$Group)
sampleinfo$Group3[sampleinfo$Group3 %in% sprintf('IP_CPEB%d',c(2,4))] <- 'IP_CPEB24'
...
###########################################################################
########## 3.1. Romer with maxmean statistic, IP vs IP
###########################################################################
...
covar.sub <- covar[covar$Group2 %in% c('IP_CPEB234','IP_CPEB1'),]
for (i in names(covar.sub)) covar.sub[,i] <- as.factor(as.character(covar.sub[,i]))
covar.sub$Group <- relevel(covar.sub$Group,ref='IP_CPEB1')
form <- "~ Group2"
...
## report directories
dir.create(paste0(res,'IP_CPEB234_vs_IP_CPEB1'))
res4 <- paste0(res, "IP_CPEB234_vs_IP_CPEB1/romer_Rdataall/")
res3 <- paste0(res, "IP_CPEB234_vs_IP_CPEB1/romer_xlsall/")
res2 <- paste0(res, "IP_CPEB234_vs_IP_CPEB1/romer_htmlall/")
dir.create(res2)
dir.create(res3)
dir.create(res4)
...
save(romer.maxmean, file= paste0(res4, "filemaxmean_IP_CPEB234_vs_IP_CPEB1.Rdata"))

#######################
## CPEB2/4 vs CPEB3
#######################

covar.sub <- covar[covar$Group3 %in% c('IP_CPEB24','IP_CPEB3'),]
for (i in names(covar.sub)) covar.sub[,i] <- as.factor(as.character(covar.sub[,i]))
covar.sub$Group <- relevel(covar.sub$Group,ref='IP_CPEB3')
form <- "~ Group3"
design <- model.matrix(as.formula(form), data = covar.sub)
...
## report directories
dir.create(paste0(res,'IP_CPEB24_vs_IP_CPEB3'))
res4 <- paste0(res, "IP_CPEB24_vs_IP_CPEB3/romer_Rdataall/")
res3 <- paste0(res, "IP_CPEB24_vs_IP_CPEB3/romer_xlsall/")
res2 <- paste0(res, "IP_CPEB24_vs_IP_CPEB3/romer_htmlall/")
dir.create(res2)
dir.create(res3)
dir.create(res4)
...
save(romer.maxmean, file= paste0(res4, "filemaxmean_IP_CPEB24_vs_IP_CPEB3.Rdata"))
...
# Write xlsx, 234 vs 1
## dirs
res4 <- paste0(res, sprintf("IP_CPEB234_vs_IP_CPEB1/romer_Rdataall/"))
res3 <- paste0(res, sprintf("IP_CPEB234_vs_IP_CPEB1/romer_xlsall/"))
res2 <- paste0(res, sprintf("IP_CPEB234_vs_IP_CPEB1/romer_htmlall/"))
## load romer results
load(paste0(res4, "filemaxmean_IP_CPEB234_vs_IP_CPEB1.Rdata"))
for(gsetsel in gsetselALL) {
...
# Write xlsx, 24 vs 1
## dirs
res4 <- paste0(res, sprintf("IP_CPEB24_vs_IP_CPEB3/romer_Rdataall/"))
res3 <- paste0(res, sprintf("IP_CPEB24_vs_IP_CPEB3/romer_xlsall/"))
res2 <- paste0(res, sprintf("IP_CPEB24_vs_IP_CPEB3/romer_htmlall/"))
## load romer results
load(paste0(res4, "filemaxmean_IP_CPEB24_vs_IP_CPEB3.Rdata"))
for(gsetsel in gsetselALL) {
    x <- romer.maxmean[[gsetsel]]$res
    x <- data.frame(rownames(x), x)
    colnames(x)[1] <- "gene set"
...
res.goslim <- lapply(c('IP_CPEB234_vs_IP_CPEB1','IP_CPEB24_vs_IP_CPEB3'),function(cpeb)
                     {
                         ## dirs
                         res4 <- paste0(res, sprintf("%s/romer_Rdataall/",cpeb))
                         ## load romer results
...
colnames(nes) <- colnames(fdr) <- c('IP_CPEB234_vs_IP_CPEB1','IP_CPEB24_vs_IP_CPEB3')
...
## heatmap pdf
pdf(file.path(figsdir,'RoastGSEA_Heatmap_GOSLIM_IP_CPEB_vs_IP_CPEB.pdf'),width=10,height=18)
...
res.kegg <- lapply(c('IP_CPEB234_vs_IP_CPEB1','IP_CPEB24_vs_IP_CPEB3'),function(cpeb)
                     {
                         ## dirs
                         res4 <- paste0(res, sprintf("%s/romer_Rdataall/",cpeb))
                         ## load romer results
...
## heatmap pdf
pdf(file.path(figsdir,'RoastGSEA_Heatmap_KEGG_IP_CPEB_vs_IP_CPEB.pdf'),width=10,height=18)
pheatmap(nes,display_numbers=round(fdr,4))
pheatmap(nes,display_numbers=fdrsym)
dev.off()
...
for (cpeb in c('IP_CPEB234_vs_IP_CPEB1','IP_CPEB24_vs_IP_CPEB3'))
    {
        ## dirs
        res4 <- paste0(res, sprintf("%s/romer_Rdataall/",cpeb))
        res3 <- paste0(res, sprintf("%s/romer_xlsall/",cpeb))
        res2 <- paste0(res, sprintf("%s/romer_htmlall/",cpeb))
        ## load romer results
        load(paste0(res4, sprintf("filemaxmean_%s.Rdata",cpeb)))
        ##
